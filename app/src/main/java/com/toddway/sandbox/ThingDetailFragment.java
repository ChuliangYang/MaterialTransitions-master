...
public class ThingDetailFragment extends TransitionHelper.BaseFragment {
    ...
    private void initDetailBody() {
        ...
        detailBodyTextView.postDelayed(new Runnable() {
            @Override
            public void run() {
...
//        }, 500);
    ...
    public void onBeforeEnter(View contentView) {
        BaseActivity.of(getActivity()).fragmentBackround.animate().scaleX(.95f).scaleY(.95f).alpha(.6f).setDuration(Navigator.ANIM_DURATION).setInterpolator(new AccelerateInterpolator()).start();
